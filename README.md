...
AI-PLC は「**広げて → 絞る**」がある仕事なら何でも回せます。コードや企画に限りません。どれも「AIに発散させ、人が要所で選び、AIが仕上げる」同じ型です。

### 🧑‍💼 仕事・プロダクト
| やりたいこと | 発散（広げる） | → 収束（絞る・仕上げる） |
|---|---|---|
| **プロダクト施策立案** | OKRに効く施策を大量に | スコアで1本選定 → Story/Specに仕様化 |
| **要件定義** | 議論・議事録から要件を洗い出す | 受け入れ基準に収束 → 画面ワイヤフレーム |
| **市場・競合リサーチ** | 論点を広げて調査 | 比較表に収束 → 意思決定メモ |
| **事業計画・企画書** | アイデアを発散 | 事業性で絞る → 提案書 |
| **ロードマップ策定** | やりたいことを棚卸し | 優先度で並べる → 四半期計画 |

### 🏡 くらし・イベント
| やりたいこと | 発散 | → 収束 |
|---|---|---|
| **旅行プラン** | 行き先・やりたいこと候補 | 日程・予算で収束 → しおり（移動/宿/食を並行設計） |
| **イベント企画**（結婚式・誕生日・オフサイト） | 演出・会場アイデア | Go判断 → 進行台本 |
| **引っ越し・部屋づくり** | レイアウト案 | 予算・動線で収束 → 買い物リスト |
| **1週間の献立** | 食べたいもの | 栄養・在庫で収束 → 買い物メモ |

### 🎨 学び・創作
| やりたいこと | 発散 | → 収束 |
|---|---|---|
| **学習計画** | 学びたいことを広げる | 目標から逆算 → カリキュラム |
| **創作**（小説・ゲーム・動画） | 世界観・プロットを発散 | 構成に収束 → 各章に再帰分解（多階層型） |
| **キャリア/副業設計** | 選択肢を発散 | 制約で絞る → 3ヶ月アクションプラン |
