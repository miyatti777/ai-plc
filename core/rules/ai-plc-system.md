> 🏷️ **Project:** [Project Palma](N/A (Project Palma - Notion))
  ...
  **Context:** AI-PLC ルートシステムルール。Claude Code の [CLAUDE.md](http://claude.md/) に相当。全SKLが暗黙的に参照する共通実行ルール・成果物構造標準・Context Cascade伝播定義を含む。
...
> 📏 **Claude Code対応:** `CLAUDE.md`（ルートシステム設定）
  ...
  **旧AIPO対応:** [Untitled](https://www.notion.so/d09d08db33714bdc9020d3ff71308c0a)
...
AI-PLCパイプラインの全スキルが従う**共通実行ルール**を定義する。Claude Codeにおける `CLAUDE.md` に相当し、プロジェクト横断で適用される基盤設定。
...
| --- | --- | --- | --- |
| global_immutable | 親→子に不変伝播。子は変更不可 | vision, tech_stack, core_principles | CLAUDE.md のルートルール |
...
1. **Sub-Layer作成時に親のglobal_immutableを自動継承**
1. **overridableは子がオーバーライドした場合のみ上書き**
1. **local_onlyは子に絶対に伝播しない**
...
> 🎨 **全Layerページの標準構造**
  ...
  1. **AI-PLC管理セクション（下部）** — 実行管理ファイル群（**必ずトグルで折りたたみ**）
...
| Subagents/ | —（新規） | AGENTS.md | ⭕ | 🤖 |
| Documents/ | Documents/ | — | ⭕ | 📄 |

---

## 4. Mob Checkpoint（旧HITL）共通ルール

> 🙋 **全スキル共通のMob Checkpointルール**

  1. **各Mob Checkpointでは必ず人間のアクションを待つ**
  1. 人間の返答があるまで次のPhaseに進まない
  1. 承認パターン: `OK` / `修正指示` / `スキップ`
  1. チーム（Mob）での判断を標準オプションとして提供
...
```mermaid
graph TD
    RUL["RUL_plc_system<br>（本ページ）"] --> SKL["全SKL_plc_*"]
    RUL --> SESSION["RUL_plc_session"]
    RUL --> ADAPTIVE["RUL_plc_adaptive"]
    RUL --> IMPL["RUL_plc_implementation"]
    SKL --> ROLES["Roles/<br>（視点テンプレート）"]
    SKL --> TEMPLATES["Skills/Templates/<br>（コマンドテンプレート）"]
    SKL --> SUBAGENTS["Subagents/<br>（Sub-Agent定義）"]
...
| サブエージェント | AGT_plc_ | AGT_plc_research | AGENTS.md |
...
- SKL実行時、[memory.md](.claude/memory.md)の関連セクションを参照して過去の知見を活用
- [soul.md](.claude/soul.md)の行動原則に常に従う
- [user.md](.claude/user.md)のユーザーモデルに基づいて対応を調整

### [memory.md](http://memory.md/)書き込みルール

**追記条件（**[**以下の場合にmemory.md**](http://xn--memory-od4exa2439dj1bmwxfgm.md/)**に追記）:**

1. バグを発見・修正したとき → 「AI-PLC運用知見」に追記
1. ユーザーの新しい判断パターンを学んだとき → 「miyattiの判断パターン」に追記
1. PJ横断で再利用可能なパターンを発見 → 「プロジェクト横断の学び」に追記
1. Notion AI固有の制約・Tipsを発見 → 「Notion AI固有の知見」に追記
...
- [ ] **Project Registry DB更新:** PJ全タスク完了時のみ、[AI-PLC Projects](https://www.notion.so/8f5680ace0254d3e9d82c260b4a1fc73) DBの該当PJのステータスを「completed」に更新
...
context:                                # 生成元コンテキスト
  source_project: "Project Palma"       # どのPJから生まれたか
  source_layer: "SG2"                   # どのLayerから
  source_task: "T015"                   # どのタスク実行中に発見したか
  decision_ref: "T003_コア原理再定義書"  # 判断根拠ドキュメント
...
1. **auto_create=true の場合:** backlogに新タスクが追加されたとき外部にもアイテム作成
1. **同期結果をログ出力:**「✅ External Sync: [type] [target] — [タスクID] を [ステータス] に更新」
...
**デフォルトDB:** [AI-PLC Tasks](https://www.notion.so/a4df4cf09a5841a4817a284f76b8d9f0)（`.notion`配下）

### 参考実装: LPALMA Product Backlog DB

[Product Backlog](https://www.notion.so/8c74174309c2466b941cf560973140de) DBとの連携パターン:

```yaml
sync_targets:
  - type: notion_db
    target_url: "https://www.notion.so/8c74174309c2466b941cf560973140de"
    mapping:
      title: "タスク名"
      status: "ステータス"
      output: "成果物"
...
> 🧹 **Karpathy Second BrainのLintワークフロー。**
  ...
  **環境別実現:** Notion→カスタムエージェント定期実行 / CC→cron / Palma→WFスケジュール
...
1. **関連トピックを特定** — 新情報のキーワード・カテゴリから該当トピックを選定
1. **既存トピックに追記** — フォーマット: `- [YYYY-MM-DD] [Source: 収集元] [内容]`
1. **バックリンク追加** — 更新したトピックの「関連トピック」セクションに相互リンクを追加
1. **新規トピック作成**（必要な場合） — frontmatter付きで作成、[index.md](http://index.md/)に行追加
1. [**log.md**](http://log.md/)**にエントリ追加** — `| YYYY-MM-DD | ingest | [ソース名] | [影響トピック数]トピック更新 |`
1. **frontmatter更新** — 影響を受けたトピックの`last_updated`と`source_count`を更新
...
> 🔗 **wikiトピックページを更新したら、必ずバックリンクを確認:**
  ...
  1. 逆方向のバックリンクも確認（A→Bを追加したらB→Aも確認）
  1. 新規トピック作成時は、関連する既存トピック全てにバックリンクを追加
...
1. **CONTRADICTIONブロッククォートを挿入** — 既存主張の直下に配置
1. **Status: openで作成** — 解決は人間の判断を待つ
1. [**log.md**](http://log.md/)**に****`contradiction`****エントリ追加** — `| YYYY-MM-DD | contradiction | [トピック名] | [矛盾内容要約] |`
1. [**index.md**](http://index.md/)**の該当トピック行に⚠️マーク追加**
...
1. **解決理由を追記** — `> Resolution: [解決内容] (YYYY-MM-DD)`
1. **廃止された主張に取り消し線を追加**（削除はしない）
1. [**index.md**](http://index.md/)**の⚠️マークを除去**
...
| 🔵 接続発見 | 異なるトピック間の予想外の接続点 | Palma WFとAI-PLC Adaptive Workflowの対応関係 |

### Query還元フロー

1. **Deliver完了時に有価値Queryを判定** — 上記カテゴリに該当するか確認
1. **該当トピックに追記** — §11 Ingest Ripple手順に従う
1. **新規トピックが必要なら作成** — 十分なボリュームがある場合のみ
1. [**log.md**](http://log.md/)**に****`query-return`****エントリ追加** — `| YYYY-MM-DD | query-return | [タスクID] | [還元先トピック] |`
...
<callout icon="🏷️" color="gray_bg">
	**Project:** （projects配下の該当ページを@mention）
...
1. 🔴 **Stock/projects** — 関連プロジェクトがないか確認
1. 🟡 **チームスペース** — WS内のチームスペースを探す
1. 🟡 **Slack** — 関連する会話や情報を検索
1. 🔵 **GitHub** — 開発系の依頼ならGitHubを探す
1. 🔵 **Web検索** — 一般的な知識が必要な場合のみ
...
1. **範囲を絞ってから取得** — LIMIT必須、プレフィクスや日付で対象を絞る
1. **不安定なら既知URLベースに切り替え** — 一覧取得が落ちる場合は既知URL起点
1. **並列更新は控えめに** — まず少数件で成功パターンを確立
1. **再実行可能性を担保** — 移行済み判定を持ち、重複・破壊を避ける
...
1. **各Stageで:** 有効なExtensionの追加チェックを自動適用
1. **検証ステップ（§18）で:** Extension固有のチェック項目をL2またはL3に追加

---

## 旧CTX対応表

| RUL_plc_system | CTX_execution_rules | 変更内容 |
| --- | --- | --- |
| Context Cascade伝播ルール | （分散記載） | 新規統合。CC 3分類を明示的に定義 |
| 成果物ページ構造標準 | 成果物ページ構造標準 | ファイル名をモダン用語に更新（intent.yaml等） |
| Mob Checkpoint共通ルール | — | 新規追加。旧HITLルールを全体に統合 |
| 命名規則（SKL/RUL/ROL/AGT） | — | 新規追加。Claude Code互換の4カテゴリ |
| スキル参照チェーン | — | 新規追加。依存関係の可視化 |
| プロンプトテンプレート | 標準プロンプトテンプレート | 削除。各SKLの「AI実行指示」セクションに移管 |
...
### 自動ロードルール
  ...
  1. **追加ルールは各SKLの「必須コンテキスト」に記載**

### 成果物チェック

  1. タスク完了前に「成果物は作成されたか？」を確認
  1. ガイド/設計書だけで完了にしない
  1. セッション終了時に作成した成果物の数を報告

### CC伝播チェック

  1. Sub-Layer作成時にglobal_immutableが継承されているか確認
  1. overridableのオーバーライドが明示されているか確認
  1. local_onlyが子に伝播していないか確認

---

## 参照元

- [Untitled](https://www.notion.so/d09d08db33714bdc9020d3ff71308c0a) — 旧版ルール（対応元）
- [T003_コア原理再定義書](https://www.notion.so/e81ef93779fd4a5c85d83a93791e3dd5) — Context Cascade原理
- [T007_新コマンド体系アーキテクチャ設計書](https://www.notion.so/d39600b2da7142679bc22451089aeeae) — 命名規則・構造設計
...
**バージョン:** 1.1（§8 Propagationスキップ禁止化 + チェックリスト出力テンプレート追加 + §18連動明記）